...
class Utils{
	...
	// PHP 8.5 port: mcrypt removed. AES-256-ECB via openssl, zero-padded to match legacy behavior.
	const CIPHER = "aes-256-ecb";

	public static function aes_encode(string $str, string $key) :string{
		$key = self::aes_norm_key($key);
		$str = self::aes_pad($str);
		$out = openssl_encrypt($str, self::CIPHER, $key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING);
		return $out === false ? "" : $out;
	}

	public static function aes_decode(string $str, string $key) : string{
		$key = self::aes_norm_key($key);
		$out = openssl_decrypt($str, self::CIPHER, $key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING);
		if($out === false){
			return "";
		}
		return rtrim($out, "\x00");
	}

	private static function aes_norm_key(string $key) : string{
		// mcrypt RIJNDAEL_256 used a 32-byte block; AES-256 key must be 32 bytes.
		if(strlen($key) > 32){
			return substr($key, 0, 32);
		}
		if(strlen($key) < 32){
			return str_pad($key, 32, "\x00");
		}
		return $key;
	}

	private static function aes_pad(string $str) : string{
		$block = 32; // legacy RIJNDAEL_256 block size
		$pad = $block - (strlen($str) % $block);
		return $str . str_repeat("\x00", $pad);
	}
	...
	public static function getRandomBytes($length = 16, $secure = true, $raw = true, $startEntropy = "", &$rounds = 0, &$drop = 0){
		...
		while(!isset($output[$length - 1])){
			...
			foreach($weakEntropy as $k => $c){ //mixing entropy values with XOR and hash randomness extractor
				$value = (string)$value ^ (string)(hash("sha256", $c . microtime() . $k, true) . hash("sha256", mt_rand() . microtime() . $k . $c, true));
				$value = (string)$value ^ (string)hash("sha512", ((string) (mt_rand() / getrandmax())) . $c . microtime() . $k, true);
			}
			unset($weakEntropy);
			...
			if($secure === true){
				...
				$strongEntropy = (string) array_pop($strongEntropyValues);
				foreach($strongEntropyValues as $value){
					$strongEntropy = $strongEntropy ^ (string) $value;
				}
				$value = "";
