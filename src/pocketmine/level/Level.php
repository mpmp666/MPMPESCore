...
class Level implements ChunkManager, Metadatable{
	...
	*/
	...
	public function dropItem(Vector3 $source, Item $item, ?Vector3 $motion = null, $delay = 10){
		$motion = $motion === null ? new Vector3((mt_rand() / getrandmax()) * 0.2 - 0.1, 0.2, (mt_rand() / getrandmax()) * 0.2 - 0.1) : $motion;
		...
		if($item->getId() > 0 and $item->getCount() > 0){
			$itemEntity = Entity::createEntity("Item", $this->getChunk($source->getX() >> 4, $source->getZ() >> 4, true), new CompoundTag("", [
				...
				"Rotation" => new ListTag("Rotation", [
					new FloatTag("", (mt_rand() / getrandmax()) * 360),
					new FloatTag("", 0)
				]),
	...
	public function useItemOn(Vector3 $vector, Item &$item, $face, $fx = 0.0, $fy = 0.0, $fz = 0.0, ?Player $player = null){
		...
		// 剪毛优先: 手持剪刀右键羊 -> shear() (MCPE 0.14.3 补全), 放在 AIR 检查前确保命中
		if($player !== null and $item->getId() === Item::SHEARS){
			$fx_ = floor($vector->x); $fy_ = floor($vector->y); $fz_ = floor($vector->z);
			$bb = new \pocketmine\level\AxisAlignedBB($fx_ - 0.5, $fy_ - 0.5, $fz_ - 0.5, $fx_ + 1.5, $fy_ + 2.5, $fz_ + 1.5);
			foreach($this->getNearbyEntities($bb) as $e){
				if($e instanceof \pocketmine\entity\Sheep and $e->shear($player)){
					return true;
				}
			}
		}
