...
class Hopper extends Spawnable implements InventoryHolder, Container, Nameable{
	...
	/** @var int 漏斗传输冷却(8 tick), 用纯 PHP 属性而非 NBT 字段, 避免序列化/反序列化引入 NPE */
	...
	public function canUpdate(){
		return $this->transferCooldown === 0 and !$this->isPowered;
	}
	...
	/**
	 * 在 inventory 中找第一个非 AIR 物品 (通用方法, 不依赖 firstOccupied)
	 * 兼容 ChestInventory / FurnaceInventory / DispenserInventory 等所有 BaseInventory 子类
	 *
	 * @param \pocketmine\inventory\BaseInventory $inventory
	 *
	 * @return Item|null
	 */
	private function findFirstItem($inventory){
		foreach($inventory->getContents() as $slot){
			if($slot instanceof Item and $slot->getId() !== Item::AIR and $slot->getCount() > 0){
				return clone $slot;
			}
		}
		return null;
	...
	public function onUpdate(){
		...
		//Pickup dropped items (每个 tick 只吸 1 个, 避免遍历整个 chunk 实体表)
		...
		$entities = $this->getLevel()->getChunkEntities($this->getBlock()->x >> 4, $this->getBlock()->z >> 4);
		$pickedAny = false;
		foreach($entities as $entity){
			...
			if($this->inventory->canAddItem($item)){
				...
				$entity->kill();
				$pickedAny = true;
				break; //一 tick 一物品, 防止一 tick 内吞掉整个掉落物堆
			}
		}
		...
		if($source instanceof Tile and $source instanceof InventoryHolder){
			...
			$sourceItem = $this->findFirstItem($inventory);
			if($sourceItem !== null){
				$sourceItem->setCount(1);
				if($this->inventory->canAddItem($sourceItem)){
					$this->inventory->addItem($sourceItem);
