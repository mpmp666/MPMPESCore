...
namespace pocketmine {
	...
	// PHP 8.5 port: load pthreads-compatible global shims FIRST, before any
	// class that extends \Thread / \Threaded gets required.
	require_once(__DIR__ . "/pthreads_shim.php");
	...
	// PHP 8.5 port: pthreads is replaced by a single-process shim (Thread/Worker/ThreadManager).
	...
	// PHP 8.5 port: pthreads replaced by global shim (src/pocketmine/pthreads_shim.php).
	// No pthreads extension check needed.
	...
	$killer = new ServerKiller(60);
	$killer->start();

	// On s390x + PHP 8.4 ZTS, Worker::shutdown() does not reliably wake a logger
	// thread blocked in sleep(), so $logger->join() would deadlock the main thread
	// until ServerKiller force-kills. Signal shutdown and let PHP reclaim the thread.
	// Then immediately self-terminate: the slow ZTS thread-reclaim at exit(0) makes
	// the process linger for many seconds on s390x, which looks like a hang.
	$logger->shutdown();

	echo "Server has stopped" . Terminal::$FORMAT_RESET . "\n";

	// All business threads are already signalled to stop; force a fast exit.
	if(function_exists("posix_kill")){
